...
const UserForm = ({ submitHandler, label }: UserFormProps): React.ReactElement => {
    const [email, setEmail] = useState('121727393@qq.com')
    const [password, setPassword] = useState('weekit')
...
const StyledContainer = styled(View)<{ error: boolean }>`
    ...
    ${props => {
        if (props.error === true) {
            return {
                border: `1px solid ${props.theme.palette.signal.red}`,
                backgroundColor: props.theme.palette.signal.redShade,
            }
        }

        return {
            border: `1px solid ${props.theme.palette.interactive.border}`,
...
const StyledButtonContainer = styled(TouchableOpacity)<{ disabled: boolean }>`
    background-color: ${props => (props.disabled ? '#ccc' : '#0772fa')};
    border-radius: 10px;
    padding: 10px 12px;
`
